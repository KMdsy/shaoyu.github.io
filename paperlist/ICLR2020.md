...
+ **已读**Iterative energy-based projection on a normal data manifold for anomaly localization 
  ...
  **基本流程**：
  ...
  4. 由于定位的目标是仅使得样本中异常的部分被指出。而非改变整个样本的形态到类似训练样本的样子，所以要加上正则项，控制正常版本的样本与原异常样本之间的相似度，即$L_{regularization} = \left\| x_n - x_a \right\| _0 $，然而由于L0不可导，因此使用L1作为近似，即$L_{regularization} = \left\| x_n - x_a \right\| _0 $。即用于定位异常的优化目标energy为$$E = L_{reconstruction} + \left\| x_n - x_a \right\| _0$$，上式中的x_n等同于下面表述的$x_{old}$。
